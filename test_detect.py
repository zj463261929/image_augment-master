...
#step1：设置路径、使用算法  
image_dir = "/opt/zhangjing/caffe/data/actions_new/"  #原始图片路径
xml_dir = "/opt/zhangjing/caffe/data/actions_new/"  #图片对应xml的路径
image_txt = "/opt/zhangjing/caffe/data/actions_new/benchmark.txt" #原始图片 标签对应的txt，比如文字识别：1.jpg 总结会，
																	#备注：如果处理的是文件夹下面的图片，没有这个txt文件，代码会自动生成的 
save_dir = "/opt/zhangjing/caffe/data/actions_new/benchmark/augment/" #处理后保存的路径，每个增强方法会保存在对应文件夹下
...
with codecs.open(image_txt, 'rb', "utf-8") as ann_file:
	...
	for l in lines:
		...
		if len(lst)>1:
			...
			if len(lst1)>1:
				img_name = lst1[len(lst1)-1]
			
			index = img_name[:len(img_name)-4]
			xml_path = xml_dir + lst[1] #index + ".xml"
			#print xml_path
			...
			if 1: #对比度、亮度
				...
				save_path = save_dir + "contrastAndBright/img/" + img_name1
				save_path_xml = save_dir + "contrastAndBright/xml/" + img_name[:len(img_name)-4] + "_contrastAndBright.xml"
				cv2.imwrite(save_path, result)
				write_xml(xml_path, save_path_xml, xmin,ymin,xmax,ymax, image_width, image_height)
			...
			if 0: #模糊
				...
				save_path = save_dir + "blur/img/" + img_name1
				save_path_xml = save_dir + "blur/xml/" + img_name[:len(img_name)-4] + "_blur.xml"
				cv2.imwrite(save_path, result)
				write_xml(xml_path, save_path_xml, xmin,ymin,xmax,ymax, image_width, image_height)
			...
			if 0: #噪音
				...
				save_path = save_dir + "noise/img/" + img_name1
				save_path_xml = save_dir + "noise/xml/" + img_name[:len(img_name)-4] + "_noise.xml"
				cv2.imwrite(save_path, result)
				write_xml(xml_path, save_path_xml, xmin,ymin,xmax,ymax, image_width, image_height)
			...
			if 0: #旋转,改
				...
				save_path = save_dir + "rotate/img/" + img_name1
				save_path_xml = save_dir + "rotate/xml/" + img_name[:len(img_name)-4] + "_rotate.xml"
				cv2.imwrite(save_path, result)
			
				if 1==type:
					xmin1,ymin1,xmax1,ymax1 = get_rotate_nochange(img,result,angle,xmin,ymin,xmax,ymax)
					write_xml( xml_path, save_path_xml, xmin1,ymin1,xmax1,ymax1, image_width, image_height)
				elif 0==type:
					xmin1,ymin1,xmax1,ymax1 = get_rotate_change(img,result,angle, xmin, ymin, xmax,ymax)
					write_xml( xml_path, save_path_xml, xmin1,ymin1,xmax1,ymax1, image_width, image_height)

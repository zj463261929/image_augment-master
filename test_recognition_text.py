...
#step1：设置路径、使用算法  
image_dir = "/opt/ligang/data/icdar2017rctw/train-word/"  #原始图片路径
image_txt = "/opt/ligang/data/icdar2017rctw/WordBBText_new_new.txt" #原始图片 标签对应的txt，比如文字识别：1.jpg 总结会，
																	#备注：如果处理的是文件夹下面的图片，没有这个txt文件，代码会自动生成的 
save_dir = "/opt/ligang/data/icdar2017rctw/train-word_augment/" #处理后保存的路径，每个增强方法会保存在对应文件夹下
...
with codecs.open(image_txt, 'rb', "utf-8") as ann_file:
	...
	for l in lines:
		...
		if len(lst)>0:
			...
			if 1: #平移
				x_trans = random.randint(0,3)
				y_trans = random.randint(0,3)
			...
			if 1: #变焦/拉伸
				...
				p1y = random.randint(0,5)
				p2x = img.shape[1] -1 - p1x
				p2y = img.shape[0] -1 - p1y
				#print p1x,p2x,p1y,p2y
				#print img.shape
				if p1x<img.shape[1]/3 and p1y<img.shape[0]/3 and p2x>0 and p2y>0:
